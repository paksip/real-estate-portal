...
interface RealEstateController {
    ...
    @PreAuthorize("hasRole(\"" + AuthoritiesConstants.USER + "\")")
    fun uploadFiles(@ApiParam(value = "", required = true) @PathVariable("realEstateId") realEstateId: Long, @ApiParam(value = "", required = true) @RequestParam("file") file: MultipartFile): ResponseEntity<Void>
